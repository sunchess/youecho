defmodule YouechoWeb.QuestionControllerTest do
  use YouechoWeb.ConnCase

  alias Youecho.Dialogs

  @create_attrs %{answer: "some answer", answer_pronounce: true, iterate_count: 42, question: "some question"}
  ...
  def fixture(:question) do
    {:ok, question} = Dialogs.create_question(@create_attrs)
    question
  end

  describe "index" do
    test "lists all dialogs", %{conn: conn} do
      conn = get(conn, Routes.question_path(conn, :index))
      assert html_response(conn, 200) =~ "Listing Dialogs"
    end
  end
